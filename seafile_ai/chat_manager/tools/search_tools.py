...
[
    {
        ...
        "title": "Title",
        "path": "/path/to/file",
        "ai_summary": "AI summary or search result snippet",
...
class DocumentsSearch(BasicTool):
    tool = {
        ...
        'function': {
            ...
            'description': (
                ...
                'Results contain AI summaries or matched snippets, not file content. '
                'Use read_files with the returned exact path when the file content is needed. '
    ...
    def _format_candidates(self, search_results):
        ...
        for item in search_results:
            ...
            candidates.append({
                ...
                'snippet': strip_mark_tags(item.get('snippet', '')),
    ...
    def execute(self, query, context, app, model, tool_executor, call_back):
        ...
        for index, candidate in enumerate(reranked_candidates, start=1):
            ai_summary = candidate.get('ai_summary') or candidate['snippet']
            source = {
                'type': 'seafile',
