...
def parse_md_to_spilt_sentences(content):
    ...
    recursiver_splitter = RecursiveCharacterTextSplitter(chunk_size=chunk_size, chunk_overlap=0)

    sentences = []
    for data in md_header_splits:
        text = data.get('content', '')
        if len(text) > chunk_size:
            split_texts = recursiver_splitter.split_text(text)
...
class Extractor(object):
    ...
    def extract(self, repo_id, version, obj_id, path):
        ...
        try:
            ...
            logger.exception('failed to extract %s: %s', path, e)
            return None
